...
5. 创建完成后，你会回到列表页。在底部的 **"服务账号"** 区域，**点击你刚才创建的账号邮箱链接** (例如 `notion-sync@...`) 进入详情页
6. 在详情页顶部，点击 **"密钥" (Keys)** 标签页 → **"添加密钥"** → **"创建新密钥"** → 选择 **JSON** → 下载文件
7. **用记事本打开下载的 JSON 文件，复制里面的全部内容** (从 `{` 开始到 `}` 结束)
8. **重要**：复制服务账号的邮箱 (即刚才点击的那个邮箱地址)，进入你的 Google Drive 文件夹，点击右上角“共享”，将此邮箱添加为编辑者
